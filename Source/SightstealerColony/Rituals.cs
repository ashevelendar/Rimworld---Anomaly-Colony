...
namespace SightstealerColony
{
    ...
    public static class VeiledOfferingUtility
    {
        ...
        public static void Resolve(PsychicRitual ritual)
        {
            ...
            quality = SightstealerUtility.Clamp01(quality + (SightstealerUtility.IsDeepDark(invoker) ? 0.2f : 0.08f));
            if (quality < 0.65f && Rand.Chance(0.05f))
            {
                victim.Kill(null); ApplyFailureEffects(participants, true);
                Messages.Message("The Veiled Offering consumes the prisoner, but nothing emerges.", MessageTypeDefOf.NegativeEvent, false); return;
            }
            if (quality < 0.35f)
            {
                victim.Kill(null); ApplyFailureEffects(participants, true);
                Messages.Message("The Veiled Offering collapses into psychic ruin.", MessageTypeDefOf.NegativeEvent, false); return;
            }
        ...
        private static Pawn GenerateReplacement(Pawn invoker)
        {
            ...
            request.Context = PawnGenerationContext.NonPlayer; request.KindDef = kind; request.Faction = invoker.Faction; request.Tile = invoker.Tile;
            request.ForceGenerateNewPawn = true; request.FixedBiologicalAge = 0f; request.FixedChronologicalAge = 0f; request.ForcedXenotype = xenotype;
            request.CanGeneratePawnRelations = false; request.ColonistRelationChanceFactor = 0f; request.RelationWithExtraPawnChanceFactor = 0f;
        ...
        private static void CopyInheritance(Pawn source, Pawn destination)
        {
            ...
            foreach (SkillRecord sourceSkill in source.skills.skills)
            {
                ...
                if (Rand.Chance(Rand.Range(0.2f, 0.4f))) { destinationSkill.Level = sourceSkill.Level; destinationSkill.xpSinceLastLevel = sourceSkill.xpSinceLastLevel; }
        ...
        private static void ApplyFailureEffects(List<Pawn> participants, bool severe)
        {
            foreach (Pawn participant in participants)
            {
                ...
                SightstealerUtility.AddHediff(participant, "PsychicComa"); SightstealerUtility.AddHediff(participant, "SS_RitualBlindness"); SightstealerUtility.AddHediff(participant, "SS_RitualWound");
    ...
    public class CompAbilityEffect_SSTeleport : CompAbilityEffect_Teleport
    {
        ...
        {
            base.Apply(target, dest); Pawn pawn = parent.pawn; if (pawn == null) return;
            parent.StartCooldown(SightstealerUtility.IsDeepDark(pawn) ? 2500 : 4000);
        }
        private bool IsValidDestination(LocalTargetInfo target, bool throwMessages)
        {
            ...
            {
                Thing carried = pawn.carryTracker.CarriedThing; if (carried.def != null && (carried.def.size.x > 1 || carried.def.size.z > 1)) return false;
            }
            if (!target.Cell.Walkable(pawn.Map) || target.Cell.GetThingList(pawn.Map).Any(t => t is Pawn)) return false;
            float glow = SightstealerUtility.GlowAt(pawn); bool deepDark = SightstealerUtility.IsDeepDark(pawn);
