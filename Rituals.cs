...
namespace SightstealerColony
{
    public class PsychicRitualDef_VeiledOffering : PsychicRitualDef
    {
        ...
        {
            if (!SightstealerUtility.IsSightstealer(pawn))
            {
                return new AcceptanceReport("A Sightstealer must lead this ritual.");
            }
    ...
    {
        public override bool Tick(PsychicRitual ritual, PsychicRitualGraph graph)
        {
            return true;
        }
        ...
        public override void End(PsychicRitual ritual, PsychicRitualGraph graph, bool success)
        {
            if (success)
            {
                VeiledOfferingUtility.Resolve(ritual);
            }
        }
    }
    ...
    public static class VeiledOfferingUtility
    {
        ...
        {
            if (ritual == null || ritual.Map == null || ritual.assignments == null)
            {
                return;
            }
            ...
            {
                if (participant != null && participant.Spawned)
                {
            ...
            {
                if (SightstealerUtility.IsSightstealer(participant))
                {
                    invoker = participant;
                    break;
                }
            ...
            float quality = SightstealerUtility.Clamp01(ritual.PowerPercent);
            if (SightstealerUtility.IsDeepDark(invoker))
            {
                quality = SightstealerUtility.Clamp01(quality + 0.2f);
            }
            else
            {
                quality = SightstealerUtility.Clamp01(quality + 0.08f);
            }

            if (quality < 0.65f && Rand.Chance(0.05f))
            {
                victim.Kill(null);
                ApplyFailureEffects(participants, true);
        ...
        private static bool IsValidVictim(Pawn pawn)
        {
            return pawn != null && pawn.Spawned && !pawn.Dead && (pawn.IsPrisonerOfColony || pawn.IsSlaveOfColony);
        }
        ...
        private static void CopyInheritance(Pawn source, Pawn destination)
        {
            ...
            foreach (SkillRecord sourceSkill in source.skills.skills)
            {
                ...
                if (sourceSkill.passion != Passion.None && Rand.Chance(Rand.Range(0.3f, 0.5f)))
                {
        ...
        private static void ApplyFailureEffects(List<Pawn> participants, bool severe)
        {
            foreach (Pawn participant in participants)
            {
                if (participant == null || participant.Dead || participant.health == null)
                {
                    continue;
                }
                ...
                if (severe && participant.mindState != null)
                {
                    participant.mindState.mentalStateHandler.TryStartMentalState(MentalStateDefOf.PanicFlee);
                }
            }
        }
        ...
        private static IntVec3 FindSpawnCell(Pawn invoker)
        {
            ...
            {
                if (cell.x < 0 || cell.z < 0 || cell.x >= invoker.Map.Size.x || cell.z >= invoker.Map.Size.z || !cell.Walkable(invoker.Map))
                {
                    continue;
                }

                bool hasPawn = false;
                foreach (Thing thing in cell.GetThingList(invoker.Map))
                {
                    if (thing is Pawn)
                    {
                        hasPawn = true;
    ...
    {
        public CompProperties_SSTeleport()
        {
            compClass = typeof(CompAbilityEffect_SSTeleport);
    ...
    {
        public new CompProperties_SSTeleport Props
        {
            get { return (CompProperties_SSTeleport)props; }
        }

        public override bool CanApplyOn(LocalTargetInfo target, LocalTargetInfo dest)
        {
            return IsValidDestination(target, false);
        }

        public override bool Valid(LocalTargetInfo target, bool throwMessages = false)
        {
            return IsValidDestination(target, throwMessages);
        }
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            // Deep darkness gives the full 25-tile skip and a shorter practical cooldown.
            if (SightstealerUtility.IsDeepDark(pawn))
            {
                parent.StartCooldown(2500);
            }
            else
            {
                parent.StartCooldown(4000);
            }
        }
        ...
        {
            Pawn pawn = parent.pawn;
            if (pawn == null || pawn.Map == null || pawn.Downed || !target.IsValid || target.Cell.x < 0 || target.Cell.z < 0 || target.Cell.x >= pawn.Map.Size.x || target.Cell.z >= pawn.Map.Size.z)
            {
                return false;
            }
            ...
            if (pawn.carryTracker != null && pawn.carryTracker.CarriedThing != null)
            {
                ...
                if (carried.def != null && (carried.def.size.x > 1 || carried.def.size.z > 1))
                {
                    return false;
                }
            }

            if (!target.Cell.Walkable(pawn.Map) || target.Cell.GetThingList(pawn.Map).Any(t => t is Pawn))
            {
                return false;
            }
            ...
            if (glow < 0f || pawn.Position.DistanceTo(target.Cell) > maxRange)
            {
                return false;
            }

            if (glow >= SightstealerUtility.Config.veryBrightGlowThreshold)
            {
                return false;
            }
