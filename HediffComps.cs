...
namespace SightstealerColony
{
    ...
    {
        public HediffCompProperties_LightSensitivity()
        {
            compClass = typeof(HediffComp_LightSensitivity);
        }
    }
    ...
    public class HediffComp_LightSensitivity : HediffComp
    {
        ...
        public override void CompPostPostAdd(Nullable<DamageInfo> dinfo)
        {
        ...
        private void UpdateSeverity()
        {
            ...
            if (glow >= config.veryBrightGlowThreshold)
            {
                parent.Severity = 1f;
            }
            else if (glow >= config.brightGlowThreshold)
            {
    ...
    {
        public HediffCompProperties_DarkRegeneration()
        {
            compClass = typeof(HediffComp_DarkRegeneration);
        }
    }
    ...
    public class HediffComp_DarkRegeneration : HediffComp
    {
        ...
        private void UpdateAndHeal(int interval)
        {
            ...
            SightstealerConfigExtension config = SightstealerUtility.Config;
            if (darkness >= 1f - config.deepDarkGlowThreshold)
            {
                parent.Severity = 1f;
            }
            else if (darkness >= 1f - config.darkGlowThreshold)
            {
                parent.Severity = 0.5f;
            }
            else
            ...
            if (interval <= 0 || Pawn == null || Pawn.health == null || Pawn.Dead)
            {
                return;
            }

            float healingPerDay = darkness >= 1f - config.deepDarkGlowThreshold
                ? config.deepDarkHealingPerDay
                : config.darkHealingPerDay;
            ...
            // GetHediffsTendable is used deliberately so only ordinary, tendable injuries are touched.
    ...
    {
        public HediffCompProperties_ConditionalInvisibility()
        {
            compClass = typeof(HediffComp_ConditionalInvisibility);
        }
    }
    ...
    public class HediffComp_ConditionalInvisibility : HediffComp
    {
        ...
        {
            base.CompExposeData();
            Scribe_Values.Look(ref revealTicksRemaining, "revealTicksRemaining", 0);
        }
        ...
        private void UpdateVisibility()
        {
            ...
            {
                invisibility.BecomeVisible(false);
            }
        }
    }
}
